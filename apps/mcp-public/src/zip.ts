 ...
 * passam a chamá-la e estes arquivos somem.
 */
 ...
 * o mesmo ZIP — só muda a extensão do arquivo baixado.
 *
 ...
 * só em memória. Ler a skill inteira antes de compactar punha até centenas de MB
 * por download no processo — e num vMCP aberto o download é anônimo.
 *
 ...
 * dois tratamentos de erro — antes do primeiro byte a falha ainda vira resposta
 * de erro (o `throw` chega ao tratador do Express); depois dele o status já foi,
 * e só resta derrubar a conexão para o cliente não guardar um `.zip` truncado
 * com cara de íntegro.
 */
export async function streamSkillZip(
  ...
  try {
    ...
    console.error('[mcp-public] erro ao gerar pacote:', (err as Error).message);
